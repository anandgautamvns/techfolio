    ...
    <div className="py-10 max-w-3xl">
      ...
      <div className="space-y-5">
        {(posts ?? []).map((post) => (
          <article
            key={post.id}
            className="group p-6 rounded-xl bg-slate-800/40 border border-slate-700/40 hover:border-slate-600/60 hover:bg-slate-800/60 transition-all cursor-pointer"
          >
            <div className="flex items-start justify-between gap-3 mb-3">
              <span className={`px-2.5 py-0.5 rounded-full text-xs font-medium border ${categoryColors[post.category] ?? "text-blue-400 bg-blue-400/10 border-blue-400/20"}`}>
                {post.category}
              </span>
            ...
            <h2 className="text-white font-semibold text-base mb-2 group-hover:text-blue-400 transition-colors">
              {post.title}
            </h2>
