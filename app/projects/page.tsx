    ...
    <div className="py-10 max-w-5xl">
      ...
      <div className="grid sm:grid-cols-2 xl:grid-cols-3 gap-5">
        {filtered.map((project) => (
          <div
            key={project.id}
            className="group flex flex-col rounded-xl bg-slate-800/40 border border-slate-700/40 hover:border-slate-600/60 overflow-hidden transition-all hover:-translate-y-0.5"
          >
            {/* Image placeholder */}
            <div className={`h-40 bg-gradient-to-br ${gradients[project.category] ?? "from-slate-700/30 to-slate-800/30"} flex items-center justify-center border-b border-slate-700/40`}>
            ...
            <div className="flex flex-col flex-1 p-5">
              ...
              <p className="text-slate-400 text-xs leading-relaxed mb-4 flex-1">{project.description}</p>
              ...
              <div className="flex items-center gap-3">
                <a href={project.github} target="_blank" rel="noopener noreferrer" className="flex items-center gap-1.5 text-slate-400 hover:text-white text-xs transition-colors">
                  <GithubIcon size={13} /> Code
                </a>
                <a href={project.demo} target="_blank" rel="noopener noreferrer" className="flex items-center gap-1.5 text-blue-400 hover:text-blue-300 text-xs transition-colors">
